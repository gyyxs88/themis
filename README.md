...
- 更完整的自动化测试覆盖；当前已覆盖 Web modules、CLI、Feishu、server、core、context、memory、diagnostics 与 mcp 的基础用例，`session store`、`fork transcript` 与 `history API` 这轮已补齐，后续重点收敛到 `router` 与 `Web stream` 主链路。
