...
- `持久化 agent` 的 `P3 / 自动创建与治理`、`P4 / 协作与交接`、`P5 / 运行边界与执行` 已全部收口；`P6 / manager 治理面` 也已在 `2026-04-08` 完成第二轮治理工作台化：组织级治理摘要、共享筛选、manager 热点、组织级 waiting queue / collaboration dashboard 联动，以及总览层的“查看等待项 / 查看父任务 / 暂停或恢复 manager”等最小治理动作都已接进 Web `Agents` 面板。随后同日又完成了“更强物理隔离首刀”：长期 agent 现在会使用自己独立的 `CODEX_HOME` 运行目录，路径固定在 `infra/local/managed-agents/<agentId>/codex-home`；auth 模式会从认证账号 home 复制鉴权文件并复用技能目录，third-party 模式也会复用独立 home，从而把 app-server 的运行态、配置和本地历史真正按 agent 分开。服务层现已固定 `getOrganizationGovernanceOverview(...)`、共享治理筛选和增强版组织级 waiting / collaboration DTO；HTTP 现已包含 `POST /api/agents/execution-boundary/update`、`POST /api/agents/governance-overview`、`POST /api/agents/waiting/list` 与 `POST /api/agents/collaboration-dashboard`，其中后面三条会共享同一套治理筛选语义。当前没有新的阻塞型主线；远端执行节点方向已经完成评估，但目前暂缓，不作为当前路线图 open 主线。
- 项目级 CLI 提供 `init / status / doctor / config / auth / skill / mcp-server` 等运维入口；`doctor smoke web|feishu|all` 已成为正式的低成本真实链路复跑入口，`doctor release` 也已成为统一发布就绪检查，diagnostics 与 HTTP 共用数据源，`doctor service` 也已开始直接暴露最近多模态输入的 compile 事实摘要。
...
- `自然语言定时任务 / 首刀基础设施` 已落地：后端现已支持单次定时任务的创建 / 列表 / 取消 / 到点执行，并新增 `themis mcp-server`，通过 MCP 工具向 Codex 暴露 `create_scheduled_task / list_scheduled_tasks / cancel_scheduled_task`。当前这仍是“外部调度 + Codex 执行”的首刀形态；Web / 飞书对话主链已自动注入这组工具与使用提示，飞书到点执行结果也会主动回推到原 chat，Web 侧则继续通过原 session 历史可见。
- `云端 / 远程执行能力评估` 已完成：当前结论仍是 Themis 如需引入远程 / 共享执行节点，应优先沿“远端 websocket `codex app-server`”方向推进；`codex cloud` 暂不作为现有 Web / 飞书聊天主链。但这条方向当前已明确暂缓，只有在后续出现共享执行节点、跨机器工作区或长任务托管需求时再重新立项。
