...
- 阶段 4 已完成前三刀：第一刀共享 PDF 资产加工已把 Web 上传与飞书下载到的 PDF 统一回填 `textExtraction` 与 `pageCount`；第二刀已把 Web 新发送与历史回放的输入资产预览闭环补上，turn 现在会显式承载 `inputEnvelope`，并在回放里展示文件名、类型、PDF 页数、文本预览与加工状态；第三刀则把常见 UTF-8 文本型文档也接入共享输入资产层，Web 上传与飞书下载到的文本文档现在会统一补 `textExtraction.textPath / textPreview`。
- 长期记忆现已补齐独立候选池与人工确认流；正式主记忆和候选建议分开持久化，`/api/actors/memory-candidates/*` 已支持 suggest / list / approve / reject / archive。
- 项目级 CLI 提供 `init / status / doctor / config / auth / skill` 等运维入口；`doctor smoke web|feishu|all` 已成为正式的低成本真实链路复跑入口，diagnostics 与 HTTP 共用数据源。
...
- `doctor`：默认总览页现在会直接输出 `feishu / mcp` 异常热点和建议先看的诊断命令，适合先判断问题更像落在哪一层。
- `doctor mcp`：会输出 MCP server 状态分布、每个 server 的分类/关键细节/建议动作，以及总诊断，先回答“是不是 app-server / MCP 装载层出问题了，具体卡在哪个 server”。
- `doctor smoke web`：自动验证真实 Web / HTTP 主链路是否能进入 `task.action_required`，并在补充输入后收口为 `completed`。
