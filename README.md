...
当前 `themis-worker-node` 独立仓已经承接最小 `worker-node run` daemon 闭环、`doctor worker-node` 预检，以及会生成本地 `report.json` 的本机执行器；下一顺序差口已切到 `systemd` 模板/部署文档迁入与本地 runtime / `credential / provider` 装配继续收口。
