...
- `持久化 agent` 的 `P3 / 自动创建与治理`、`P4 / 协作与交接`、`P5 / 运行边界与执行` 已全部收口，`P6 / manager 治理面 / 组织级跨父任务汇总台` 也已在 `2026-04-07` 落地：自动创建、护栏、审计、bootstrap onboarding、idle recovery、`handoff` 时间线、父子 `work item` 汇总、组织级 waiting queue、组织级跨父任务协作汇总台都已接进主链；执行边界也已经真正落到运行时，当前每个长期 agent 都会持久化自己的默认 `workspace policy` 和 `runtime profile`，新派工会自动继承 `workspacePolicySnapshot / runtimeProfileSnapshot`，执行时会校验工作区、写入 session workspace、合并附加目录、收紧网络开关，并按 `auth / third-party provider` 隔离 `app-server` 会话环境。HTTP 现已包含 `POST /api/agents/execution-boundary/update` 与 `POST /api/agents/collaboration-dashboard`，Web `Agents` 面板也可直接查看默认执行边界和组织级跨父任务汇总。当前没有新的阻塞型主线；如果后续继续深挖持久化 agent，优先转到下一轮更重的 manager 治理面、物理隔离或远端执行节点，而不是继续扩第二套壳层。
