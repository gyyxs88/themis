...
主 Themis 里的 `Platform Agents` 当前也已收口成纯兼容 gateway：`/api/agents/list` 只负责暴露平台兼容状态与跳转上下文；如果没有有效的 `THEMIS_PLATFORM_*` 配置，其余 `/api/agents/*` 不再回退本地平台治理，而会明确提示去配置平台 gateway 或直接使用独立 `themis-platform` 页面。
