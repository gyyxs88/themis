...
- 提供单次与 `daily / weekly / monthly` 重复定时任务、员工治理、Themis 密码本、worker secret 准备与运营中枢机器协议，并通过 `themis mcp-server` 暴露 `create_scheduled_task / list_scheduled_tasks / cancel_scheduled_task`、`list_managed_agents / get_managed_agent_detail / create_managed_agent / update_managed_agent_card / update_managed_agent_execution_boundary / dispatch_work_item / manage_themis_secret / provision_cloudflare_worker_secret / update_managed_agent_lifecycle`，以及 `list_operation_objects / create_operation_object / update_operation_object / list_operation_edges / create_operation_edge / update_operation_edge / query_operation_graph / get_operations_boss_view` 给 Codex 调用；`dispatch_work_item` 支持 `readOnlyFactSourcePacks`，可把 `cloudflare_readonly / operations_ledger_readonly / feishu_base_readonly` 这类只读事实源、运行边界和 secret 引用结构化进工单。
...
- Themis 密码本 / Worker secret 注入：`THEMIS_SECRET_STORE_FILE` 可覆盖主 Themis 本地密码本；飞书单聊支持自然语言 secret intake，用户可以把平台管理 token 直接交给 Themis 保存，Cloudflare 管理 token 会归一为 `cloudflare-management-token`，飞书 Open Platform 凭据会归一为 `feishu-app-id` / `feishu-app-secret`，Account Owned API Token 还需要在密码本保存 `cloudflare-account-id`（或配置 `THEMIS_CLOUDFLARE_ACCOUNT_ID` / `CLOUDFLARE_ACCOUNT_ID`），以便 `provision_cloudflare_worker_secret` 走 `/accounts/{account_id}/tokens`；其他平台可用“保存为 `<secretRef>`”指定引用名；Themis 自己通过 MCP 工具 `manage_themis_secret` 增删改查密码本，不要求人类使用斜杠命令；`THEMIS_MANAGED_AGENT_WORKER_SECRET_STORE_FILE` 可覆盖主 Themis 写入 worker secret store 的路径，默认指向同级 `../themis-worker-node/infra/local/worker-secrets.json`；当需要把 secret 送到指定 worker node 时，`provision_cloudflare_worker_secret` 可传 `targetNodeIds`，主 Themis 会通过平台 `/api/platform/worker/secrets/push` 下发，worker 拉取后写入本机 secret store，并通过 `secretCapabilities` 心跳参与后续调度匹配。
