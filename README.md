...
- 阶段 4 已完成前两刀：第一刀共享 PDF 资产加工已把 Web 上传与飞书下载到的 PDF 统一回填 `textExtraction` 与 `pageCount`；第二刀也已把 Web 新发送与历史回放的输入资产预览闭环补上，turn 现在会显式承载 `inputEnvelope`，并在回放里展示文件名、类型、PDF 页数、文本预览与加工状态。
