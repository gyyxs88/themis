...
- `2026-04-18` 起，主 Themis Web 设置面板已新增“内部会议室”页，并通过 `/api/meeting-rooms/*` 作为平台会议室的最小 gateway；Themis 现在可以作为唯一管理者创建会议室、查看房间消息流，并发起多轮内部讨论。
...
- 提供平台会议室 gateway 与 Web 主持台，支持 `status / list / create / detail / message/stream` 这组最小内部会议能力，让 Themis 能以管理者身份拉多个数字员工进入同一个会议室讨论。
