...
- `持久化 agent` 的 `P3 / 自动创建与治理` 已整体完成：此前已经完成 `waiting_action resume + mailbox pull`、`waiting_human gateway + human resume`、组织级等待队列与直接治理、`waiting_agent -> waiting_human` 升级、`pause / resume / archive` lifecycle 治理、安全/运行中 work item 取消收口，以及“自动创建建议器 + 自动命名最小闭环”“自动创建预算护栏 + suggestion audit”“spawn policy 持久化 + 组织级可配置护栏”“自动创建建议忽略/恢复 + 审计最小闭环”“auto-created agent bootstrap onboarding task 最小闭环”。最新这轮又把自动创建后的编制回收治理补齐成最小闭环：`ManagedAgentsService` 现在会只针对 `creationMode = auto` 的长期 agent 生成空闲回收建议，基于空闲时长、近期已收口 work item、近期 `handoff` 事实，以及“当前没有 open work item / pending mailbox”这组条件给出 `pause / archive` 建议；HTTP 已新增 `/api/agents/idle-suggestions`、`/api/agents/idle-approve`，Web `Agents` 面板也已能直接批准回收建议并查看审计记录。`P4 / 持久化 agent / 协作与交接` 的前两刀也已完成：`handoff` 已升级成一等对象并接出交接时间线，`父子 work item + manager 汇总视图` 也已接进 `work item detail` 与 Web `Agents` 详情页；当前下一步转向 `P5 / 持久化 agent / 运行边界与执行`，优先让 `workspacePolicySnapshot / runtimeProfileSnapshot` 真正驱动 agent 级目录权限、网络策略和执行隔离。
