...
- 当前全局默认运行模型是 `gpt-5.5`，默认思维强度是 `xhigh`；如需偏离，改会话设置、运行边界或底层 `config.toml` 即可。
...
- 提供单次与 `daily / weekly / monthly` 重复定时任务、员工治理、Themis 密码本、worker secret 准备与运营中枢机器协议，并通过 `themis mcp-server` 暴露 `create_scheduled_task / list_scheduled_tasks / cancel_scheduled_task`、`list_managed_agents / get_managed_agent_detail / get_work_item_detail / create_managed_agent / update_managed_agent_card / update_managed_agent_execution_boundary / dispatch_work_item / manage_themis_secret / provision_worker_secret / update_managed_agent_lifecycle`，以及 `list_operation_objects / create_operation_object / update_operation_object / list_operation_edges / create_operation_edge / update_operation_edge / query_operation_graph / get_operations_boss_view` 给 Codex 调用；`get_work_item_detail` 可直接读取数字员工工单状态、runs、completion 与父子协作摘要；`dispatch_work_item` 支持 `readOnlyFactSourcePacks`，可把 `cloudflare_readonly / operations_ledger_readonly / feishu_base_readonly` 这类只读事实源、运行边界和 secret 引用结构化进工单。
...
- MCP OAuth：`THEMIS_MCP_OAUTH_CALLBACK_BASE_URL` 可配置公开 callback base URL；未配置时仅会在 `THEMIS_BASE_URL` 是非本机回环地址时自动启用公开 callback bridge
...
- 正式机源码保护：`THEMIS_SOURCE_EDIT_POLICY=todoist-task` 可强制普通 Web / 飞书任务在需要修改仓库源码时先创建 Todoist 开发任务；未配置时，运行在 `~/services/themis-prod` 或 `themis-prod.service` 的正式实例会自动启用这条保护。
- Themis 密码本 / Worker secret 注入：`THEMIS_SECRET_STORE_FILE` 可覆盖主 Themis 本地密码本；飞书单聊支持自然语言 secret intake，用户可以把平台管理 token 直接交给 Themis 保存，Cloudflare 管理 token 会归一为 `cloudflare-management-token`，飞书 Open Platform 凭据会归一为 `feishu-app-id` / `feishu-app-secret`，其他平台可用“保存为 `<secretRef>`”指定引用名；Themis 自己通过 MCP 工具 `manage_themis_secret` 增删改查密码本，不要求人类使用斜杠命令；`provision_worker_secret` 是通用 worker 凭据下发器，默认 `mode=themis_secret`，可把任意 Themis 密码本 secretRef 写入 worker secret store，也可带 `targetNodeIds` 通过平台 `/api/platform/worker/secrets/push` 下发；Cloudflare 只读 token 需要由管理 token 派生时才使用 `mode=cloudflare_readonly`，Account Owned API Token 还需要在密码本保存 `cloudflare-account-id`（或配置 `THEMIS_CLOUDFLARE_ACCOUNT_ID` / `CLOUDFLARE_ACCOUNT_ID`），以便走 `/accounts/{account_id}/tokens`；`THEMIS_MANAGED_AGENT_WORKER_SECRET_STORE_FILE` 可覆盖主 Themis 写入 worker secret store 的路径，默认指向同级 `../themis-worker-node/infra/local/worker-secrets.json`；worker 拉取后写入本机 secret store，并通过 `secretCapabilities` 心跳参与后续调度匹配。
...
普通 Codex 对话任务不应直接尝试重启当前 Themis 服务；如果用户在飞书里要求“部署生效 / 重启自己 / 重启当前实例”，应提示用户在单聊发送 `/ops restart confirm`，由受控运维命令请求重启当前服务；重启前后状态用 `/ops status` 查看。用户可以像交给同事一样在飞书单聊里把平台 token 交给 Themis；Feishu 入口会先用通用 secret intake 截获、脱敏日志并写入 Themis 密码本，不再进入 Codex 对话、工单正文、contextPacket 或员工报告。Themis 自己通过 `manage_themis_secret` 对密码本做增删改查；worker 侧凭据统一由 `provision_worker_secret` 下发，Discord/GitHub/Feishu 等已有 secretRef 走 `mode=themis_secret`，Cloudflare 只读 token 需要本地管理 token 派生时走 `mode=cloudflare_readonly`，随后派工只传 `runtimeProfileSnapshot.secretEnvRefs` 引用；飞书 Base/多维表只读巡检所需的 Feishu App ID / App Secret 应通过单聊自然语言 intake 保存为 `feishu-app-id` / `feishu-app-secret`，不要在普通任务里跑 `lark-cli config init` 这类需要终端进程存活的授权流程；飞书 `/secrets worker set` 只作为显式选择的 worker 本机兜底入口，不作为正常 secret 管理方式，也不为每个平台新增斜杠命令。
...
- Web 已支持“运行参数 -> 实例升级”，飞书也已支持 `/update`、`/update apply confirm`、`/update rollback confirm`；单独重启当前服务使用飞书单聊 `/ops restart confirm`，实例状态和最近一次重启确认用 `/ops status` 查看；Themis 密码本由 `manage_themis_secret` 工具管理，`/secrets worker` 只是 worker secret 引用的兜底维护入口，worker secret 优先由通用 `provision_worker_secret` 准备和下发；Cloudflare 只读 token 派生场景使用 `mode=cloudflare_readonly` 并依赖 Themis 持有的管理 token 和 `cloudflare-account-id`。
