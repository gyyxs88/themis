...
- 第一条中途文本默认会先缓存；但如果首条正文已经足够完整（例如已形成较完整段落 / 列表），桥接层会直接把它露出来，并继续保留一条新的 `处理中...` 状态消息
- 同一条 `agent_message itemId` 在已经露出正文后，不会再不断新起正文消息，而是按 `FEISHU_PROGRESS_FLUSH_TIMEOUT_MS` 这个节拍尝试更新同一条已显示正文
- soft flush 到点时，桥接层会优先截到最近的空行或句末边界，只把完整段落 / 完整句子之前的内容露出来，不会把半句硬刷到飞书
- 只有切到新的正文 item 时，桥接层才会新起下一条正文；每一轮正文下面仍保留单独的 `处理中...` 状态消息
- 如果从上次可见更新开始连续 `40s` 仍然等不到新的空行或句末，桥接层会兜底强刷当前正文，避免长时间完全没动静
- 当前默认的正文更新节拍是 `20000ms`，对应的 hard flush 兜底窗口是 `40000ms`
- 单条正文消息会记录自己的飞书编辑预算；当正文主消息接近 `20` 次编辑上限时，桥接层会暂停中途增量更新，继续保留尾部 `处理中...`，把最后一次编辑留给任务最终成稿，避免撞上飞书 `230072`
...
- 最终结果到达时，如果最后一条已显示正文与最终结果不同，桥接层会优先更新这条正文，再把尾部状态收口为 `已完成`
...
- app-server 的 `item/agentMessage/delta` 在翻译成统一 `task.progress` 时，会显式保留 `threadEventType = item.completed` 这类“正文 progress”标签，避免飞书把正文增量误渲染成“任务状态更新”卡片刷屏
- 同一个 `agent_message itemId` 的连续 delta 只会覆盖当前缓存正文，不会把每次增量都落成一条新的飞书消息；只有切到下一个正文 item、静默超时或最终结果收口时，缓存才会真正发送
...
```bash
export FEISHU_PROGRESS_FLUSH_TIMEOUT_MS="20000"
```
