...
- 当前已支持飞书文本收发、`/help`、`/sessions`、`/new`、`/use`、`/current`、`/review`、`/steer`、`/workspace`、`/group`、`/link`、`/settings` 命令树、`/update`、`/ops`、`/msgupdate`、`/quota`，以及 `/account`、`/sandbox`、`/search`、`/network`、`/approval` 这些兼容入口；其中 `/settings account` 已支持设备码登录、退出账号和取消登录，`/ops restart confirm` 负责受控请求重启当前 Themis 服务。
