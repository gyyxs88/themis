...
- waiting `user-input` 现在默认走直接回复文本，不要再把 `/reply` 当成唯一主路径；如果 scope 里同时挂着多条 `user-input`，普通文本会继续走原任务链；如果还有 `approval`，同样不会触发 takeover，审批仍建议显式 `/approve` / `/deny`。
