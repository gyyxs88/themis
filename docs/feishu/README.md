...
- Codex 在飞书里现在走“正文主消息 + 状态消息”桥接：用户发消息后先立刻返回 `处理中...`；第一条中途回复默认先缓存，但如果首条正文已经足够完整，会直接露出来并保留新的 `处理中...` 状态消息；同一 `agent_message itemId` 的连续 delta 不会一条条新起正文，而会按 `FEISHU_PROGRESS_FLUSH_TIMEOUT_MS` 的节拍更新同一条已显示正文；只有切到新的正文 item 时，才会新起下一条正文。
