...
- 当前第九刀又补上了“巡检 / 排障 / 多节点值守手册”：
  ...
  - 当前值守顺序已经固定成 `doctor worker-fleet -> systemd --user -> doctor worker-node -> nodes/list -> nodes/detail -> worker-fleet drain|offline|reclaim`
  - 多节点场景下也已固定“独立目录、独立服务名、稳定节点名、能力声明真实对齐”这套值守约定
- 当前第十刀又补上了“平台侧批量节点巡检摘要”：
  - 新增 `themis doctor worker-fleet`
  - 当前会批量读取 `nodes/list + nodes/detail`
  - 会直接汇总节点 `status / heartbeat / active lease / attention` 与建议动作，避免值班时手工拼多次平台请求
- 当前第十一刀又补上了“平台侧节点治理 CLI 闭环”：
  - 新增 `themis worker-fleet drain|offline|reclaim`
  - 当前会复用 Web Access token 登录平台，不再要求值班时手工取 cookie 后拼 `curl`
  - 支持对多个 `nodeId` 顺序执行治理动作，并逐节点输出成功/失败摘要
- 到这一步，`Phase 3 / 远端执行闭环` 的当前计划范围已经完成；后续如果再扩平台，应转向新阶段，而不是继续在 `Phase 3` 里反复补运维尾项。
...
当前这一包已经把“协议层 -> 执行原型 -> daemon 化 -> 启动前预检 -> 失联 lease 显式回收 -> scheduler 自动回收 -> 部署与常驻说明 -> 真实 systemd 演练 / fresh credential 预检补齐 -> 巡检/排障/多节点值守手册 -> 平台侧批量节点巡检摘要 -> 平台侧节点治理 CLI”这十一个最小环节都接通；当前计划范围已经收口。
...
- 当前已完成到“独立 Worker Node daemon + `doctor worker-node` 启动前预检 + `nodes/reclaim` 显式 lease 回收 + scheduler 自动失联恢复 + 部署 / systemd 常驻文档 + 真实 `systemd --user` 演练 + 运维 runbook + `doctor worker-fleet` 平台总览摘要 + `worker-fleet drain|offline|reclaim` 治理 CLI”；就 `Phase 3 / 远端执行闭环` 当前计划范围而言，完成标准已满足
...
- `npm run typecheck`
- Worker Node 最小协议测试
- Worker Node 集群巡检摘要测试
- Worker Node 集群治理 CLI 回归
- Worker Node 预检 / 诊断测试
- 节点 reclaim 治理测试
- scheduler 自动失联恢复测试
- 平台 run 状态回传测试
