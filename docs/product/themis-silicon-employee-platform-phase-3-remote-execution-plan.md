...
- 当前下一步已经切到运维闭环剩余项：补齐节点部署方式、bootstrap 手册，以及更完整的自动失联恢复与治理动作，而不是再重复设计 daemon 协议。
...
当前这一包已经把“协议层 -> 执行原型 -> daemon 化 -> 启动前预检 -> 失联 lease 显式回收”这五个最小环节都接通；下一刀不再是重复补协议，而是继续把部署说明、bootstrap 与自动失联恢复收口。
...
- 当前已完成到“独立 Worker Node daemon + `doctor worker-node` 启动前预检 + `nodes/reclaim` 显式 lease 回收”；后续只差更完整的 daemon 部署说明、bootstrap 文档，以及自动失联恢复与治理动作
