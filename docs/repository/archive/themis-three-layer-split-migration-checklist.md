...
- `2026-04-13` 同日已补 `Phase 5` 第一刀：当前仓库已新增 `scripts/bootstrap-split-repos.sh` 与 `scripts/bootstrap-split-repos.test.ts`，把 `themis-platform / themis-main / themis-worker-node / themis-contracts` 四个 sibling repo 的最小初始化流程固定成可复跑脚本；同时又新增 `docs/repository/archive/themis-three-layer-phase-5-bootstrap-and-cutover.md`，把 CI / 发布 / 回归门槛与切换演练顺序固化下来。实机已验证：执行 `bash scripts/bootstrap-split-repos.sh ..` 后，当前仓同级目录已真实生成 `../themis-platform`、`../themis-main`、`../themis-worker-node`、`../themis-contracts` 四个 git 仓骨架；再次执行同一命令时，会按预期以“拒绝覆盖非空目录” fail-fast 收口。
...
- `2026-04-14` 同日又继续推进 `6.1.c / themis-platform 迁入 runtime 持久化` 第一刀：当前 `../themis-platform` 已新增 `platform-runtime-snapshot`，并让 `src/server/platform-main.ts` 在启动时从 `THEMIS_PLATFORM_RUNTIME_SNAPSHOT_FILE`（默认 `infra/platform/runtime-state.json`）恢复 `nodes / control-plane / workflow / worker-runs`，同时通过 `createPlatformApp({ onStateMutation })` 在节点治理、agents/projects 写入、mailbox/work-items 变更、worker `pull|update|complete` 等平台写动作后立即落盘。新增 `src/server/platform-runtime-snapshot.test.ts` 已把 snapshot round-trip 锁住，`src/server/platform-app-agents-projects.test.ts` 也补上了“create agent -> dispatch” workflow 同步回归；整仓 `npm run test`、`npm run typecheck`、`npm run build` 全部通过。到这一步，平台独立仓已不再是“重启就丢平台事实”的纯内存壳；当前平台机现网切换主阻塞进一步收敛到 `MySQL shared control plane` 与真正的 `runtime store / execution runtime` 接线。
- `2026-04-14` 同日又继续推进 `6.1.c / themis-platform 接入真正的 runtime store / execution runtime` 第二刀：当前 `../themis-platform` 已新增 `platform-execution-runtime-store`，`src/server/platform-main.ts` 会默认把每个 run 的 `assigned-run.json / state.json / events.ndjson` 写入 `THEMIS_PLATFORM_EXECUTION_RUNTIME_ROOT`（默认 `infra/platform/runtime-runs`）；与此同时，`worker/runs/pull|update|complete` 与 `scheduler tick` 的 reclaim 也都已接入该 store。新增 `src/server/platform-execution-runtime-store.test.ts` 已把“assigned/status/completed 轨迹落盘”与“createPlatformApp 实际写 execution runtime store”锁住，`src/server/platform-scheduler-service.test.ts` 也补上了 reclaim 事件写盘断言；整仓 `npm run test`、`npm run typecheck`、`npm run build` 全部通过。到这一步，平台独立仓已不再缺本地 execution runtime store；当前平台机现网切换主阻塞已收敛到 `MySQL shared control plane` 与本地 shared cache / mirror flush。
- `2026-04-14` 同日又继续推进 `6.1.c / themis-platform 接入 MySQL shared control plane / mirror flush` 第三刀：当前 `../themis-platform` 已新增 `platform-shared-control-plane-store`、`platform-control-plane-mirror` 与 `platform-control-plane-bootstrap`，`src/server/platform-main.ts` 现在支持 `THEMIS_PLATFORM_CONTROL_PLANE_DRIVER=mysql`，会把本地 shared cache SQLite 落到 `THEMIS_MANAGED_AGENT_CONTROL_PLANE_DATABASE_FILE`（默认 `infra/platform/control-plane.db`），并在 `shared cache SQLite + MySQL shared snapshot store` 之间做 bootstrap / flush / rollback；`createPlatformApp({ onStateMutation })` 也已改成真正 `await` mutation 持久化，flush 失败时会恢复本地 shared cache 和内存态。新增 `src/server/platform-shared-control-plane-store.test.ts`、`src/server/platform-control-plane-mirror.test.ts`、`src/server/platform-control-plane-bootstrap.test.ts` 已把 shared cache round-trip、runtime snapshot fallback bootstrap、mirror flush rollback 锁住，整仓 `npm run test`、`npm run typecheck`、`npm run build` 全部通过。到这一步，`6.1.c` 这条平台独立仓 scheduler/runtime + persistence + mirror 主线已经收口；后续顺序已从代码差口切到真实平台机切换与联调验收。
- `2026-04-14` 晚间已完成 `顺序 6.2 / 平台机切到 split repo themis-platform 并完成首轮验收`：平台机 `192.168.31.212` 当前已把现网 `themis-platform.service` 从 monorepo `dead9b5` 切到 split repo `~/services/themis-platform@92eabbf`，原目录已备份到 `~/services/themis-platform-monorepo-backup-20260414-215707`。这次切换还额外确认了两个真实坑：一是旧 monorepo `infra/platform/control-plane.db` 是直接表结构，不能直接当 split repo 的 `themis_platform_shared_snapshots(payload_json, saved_at)` 使用；二是旧 monorepo 并没有 `infra/local/platform-service-tokens.json` 这份新 Bearer token store。因此真实切换时除了复制 `.env.local / infra/local / infra/platform`，还必须按现网 token 补建 token store，并把旧 SQLite 控制面事实灌入 split repo 的 `runtime-state.json`、本地 shared cache SQLite 和 MySQL shared snapshot。迁移完成后，平台机当前已恢复 `1` 个 organization、`5` 个 managed agents、`3` 个 project bindings、`7` 个 work-items、`7` 个 assigned runs 和 `3` 个 nodes；平台机本机 `GET /api/health`、`POST /api/platform/agents/list|work-items/list|nodes/list`、`./themis-platform doctor worker-fleet` 以及主 Themis 机 `192.168.31.208` 跨机直连 `POST /api/platform/agents/list|work-items/list|nodes/list` 也都已复验通过。到这一步，`顺序 6` 当前主线已正式从“平台机是否能切”推进到 `6.3 / 主 Themis 机上游联调` 与 `6.4 / 三机异常收口`。
- `2026-04-14` 深夜已完成 `顺序 6.3 / 主 Themis 机按平台上游跑首轮局域网联调`：这轮先发现并修掉了一个真实阻塞，主 Themis 机 `192.168.31.208` 虽然已经配置 `THEMIS_PLATFORM_*`，但主仓 `main.ts` 之前并没有把 `/api/platform/*` 接到 `ManagedAgentPlatformGatewayClient`，导致主机本地 `POST /api/platform/agents|nodes|work-items/list` 一度读到本地空控制面。当前这条问题已通过开发仓 `57c8a22`、公开仓 `7e07939` 修复：主仓新增 `platform gateway facade` 装配，`main.ts` 现已在配置 `THEMIS_PLATFORM_*` 时把 `/api/platform/*` 真正切到平台上游，并补齐 `nodes/*`、`worker/runs/*` 契约读写与 HTTP 回归。主机 `192.168.31.208` 拉起 `7e07939` 后，局域网直接登录主 Themis 已复验通过：`POST /api/agents/list` 继续返回 `404 ROUTE_NOT_FOUND`，而主机本地 `POST /api/platform/agents/list` 返回 `1` 个 organization / `5` 个 agents，`POST /api/platform/work-items/list` 返回 `8` 条 work-items，`POST /api/platform/projects/workspace-binding/list` 返回 `3` 条 bindings；随后又通过主机本地 `POST /api/platform/work-items/dispatch` 新建 `work-item-platform-1`，并在不到 1 秒内通过主机本地 `POST /api/platform/runs/list|detail` 回看到了 `run-platform-1 -> node-4pjylh69 -> completed`，同时平台机直连 `POST /api/platform/work-items/detail|runs/detail` 也确认同一条记录已经落到平台事实里。这轮还额外确认了一个边界：现网 `platform-gateway-main` Bearer token 当前**无权**访问 `nodes/*`，因此主 Themis 本地 `POST /api/platform/nodes/list` 会稳定返回 `CORE_RUNTIME_ERROR / 当前平台服务令牌无权访问该接口。`；节点值班与 `worker-fleet` 继续只归独立 `themis-platform`。到这一步，`顺序 6` 当前主线已从 `6.3` 收敛到 `6.4 / 三机真实联调记录与异常收口`。
