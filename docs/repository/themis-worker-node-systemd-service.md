...
需要给 worker 注入第三方 API token 时，不要把值写进工单正文。Worker 执行前会读取本机 secret store，默认路径是 `infra/local/worker-secrets.json`；主 Themis 可通过 MCP 工具 `provision_worker_secret` 通用下发任意 Themis 密码本 secretRef，并写入同级部署下的 `../themis-worker-node/infra/local/worker-secrets.json`；Cloudflare 只读 token 需要由管理 token 派生时才使用 `mode=cloudflare_readonly`。飞书单聊 `/secrets worker set <secretRef> <secretValue>` 只作为兜底手工入口；派工始终只传 `runtimeProfileSnapshot.secretEnvRefs` 引用。
