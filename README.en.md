...
- One-off scheduled tasks, managed-employee governance, the Themis password book, worker secret provisioning, and operations-center machine protocol. `themis mcp-server` exposes `create_scheduled_task / list_scheduled_tasks / cancel_scheduled_task`, `list_managed_agents / get_managed_agent_detail / get_work_item_detail / create_managed_agent / update_managed_agent_card / update_managed_agent_execution_boundary / dispatch_work_item / manage_themis_secret / provision_worker_secret / update_managed_agent_lifecycle`, plus `list_operation_objects / create_operation_object / update_operation_object / list_operation_edges / create_operation_edge / update_operation_edge / query_operation_graph / get_operations_boss_view` for Codex.
...
- Themis password book / worker secret injection: `THEMIS_SECRET_STORE_FILE` overrides the local Themis password book. Feishu p2p messages support natural-language secret intake, so a user can hand platform tokens directly to Themis; Cloudflare management tokens normalize to `cloudflare-management-token`, and Feishu Open Platform credentials normalize to `feishu-app-id` / `feishu-app-secret`. `provision_worker_secret` is the generic worker credential delivery tool: by default `mode=themis_secret` copies any Themis password-book secretRef into the worker secret store or pushes it to target nodes; Cloudflare read-only token derivation uses `mode=cloudflare_readonly` and Account Owned API Tokens also need `cloudflare-account-id` in the password book, or `THEMIS_CLOUDFLARE_ACCOUNT_ID` / `CLOUDFLARE_ACCOUNT_ID`, so Themis can use `/accounts/{account_id}/tokens`. Other platforms can use "save as `<secretRef>`" to name the reference. Themis manages that book through the MCP tool `manage_themis_secret`, so normal secret management does not require user-facing slash commands. `THEMIS_MANAGED_AGENT_WORKER_SECRET_STORE_FILE` overrides the worker secret store path written by Themis; by default it targets sibling `../themis-worker-node/infra/local/worker-secrets.json`.
...
- Web supports "runtime settings -> instance update"; Feishu supports `/update`, `/update apply confirm`, `/update rollback confirm`, `/ops status`, `/ops restart confirm`, and generic natural-language secret intake in p2p messages. Those token handoffs are intercepted, logged redacted, and written to the Themis password book before they can become Codex prompts, work-item text, context packets, or employee reports. Themis manages the book through `manage_themis_secret`; worker secrets should normally be prepared by the generic `provision_worker_secret` tool. Existing Discord/GitHub/Feishu/etc. secretRefs use `mode=themis_secret`; Cloudflare read-only token derivation uses `mode=cloudflare_readonly` with the Themis-held management token and `cloudflare-account-id`. Feishu Base read-only inspections should receive Feishu App ID / App Secret through p2p intake as `feishu-app-id` / `feishu-app-secret`, not by running `lark-cli config init` inside a chat task; `/secrets worker` remains only a fallback worker-secret entry, not a per-platform or normal secret-management command surface.
