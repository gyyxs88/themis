...
- Themis password book / worker secret injection: `THEMIS_SECRET_STORE_FILE` overrides the local Themis password book. Feishu p2p messages support natural-language secret intake, so a user can hand platform tokens directly to Themis; Cloudflare management tokens normalize to `cloudflare-management-token`, and Feishu Open Platform credentials normalize to `feishu-app-id` / `feishu-app-secret`. Account Owned API Tokens also need `cloudflare-account-id` in the password book, or `THEMIS_CLOUDFLARE_ACCOUNT_ID` / `CLOUDFLARE_ACCOUNT_ID`, so `provision_cloudflare_worker_secret` can use `/accounts/{account_id}/tokens`. Other platforms can use "save as `<secretRef>`" to name the reference. Themis manages that book through the MCP tool `manage_themis_secret`, so normal secret management does not require user-facing slash commands. `THEMIS_MANAGED_AGENT_WORKER_SECRET_STORE_FILE` overrides the worker secret store path written by Themis; by default it targets sibling `../themis-worker-node/infra/local/worker-secrets.json`.
