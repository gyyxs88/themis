...
- One-off scheduled tasks, managed-employee governance, worker secret provisioning, and operations-center machine protocol. `themis mcp-server` exposes `create_scheduled_task / list_scheduled_tasks / cancel_scheduled_task`, `list_managed_agents / get_managed_agent_detail / create_managed_agent / update_managed_agent_card / update_managed_agent_execution_boundary / dispatch_work_item / provision_cloudflare_worker_secret / update_managed_agent_lifecycle`, plus `list_operation_objects / create_operation_object / update_operation_object / list_operation_edges / create_operation_edge / update_operation_edge / query_operation_graph / get_operations_boss_view` for Codex.
...
- Themis / worker secret injection: `THEMIS_SECRET_STORE_FILE` overrides the local Themis secret store; the Cloudflare management token can come from `THEMIS_CLOUDFLARE_MANAGEMENT_TOKEN` or `cloudflare-management-token` in that store. `THEMIS_MANAGED_AGENT_WORKER_SECRET_STORE_FILE` overrides the worker secret store path written by Themis; by default it targets sibling `../themis-worker-node/infra/local/worker-secrets.json`.
...
- Web supports "runtime settings -> instance update"; Feishu supports `/update`, `/update apply confirm`, `/update rollback confirm`, `/ops status`, `/ops restart confirm`, and `/secrets worker` as a fallback worker secret reference entry. Cloudflare worker secrets should normally be prepared by `provision_cloudflare_worker_secret` from the Themis-held management token.
