...
- Themis password book / worker secret injection: `THEMIS_SECRET_STORE_FILE` overrides the local Themis password book. Feishu p2p messages support natural-language secret intake, so a user can hand platform tokens directly to Themis; Cloudflare management tokens normalize to `cloudflare-management-token`, and other platforms can use "save as `<secretRef>`" to name the reference. Themis manages that book through the MCP tool `manage_themis_secret`, so normal secret management does not require user-facing slash commands. `THEMIS_MANAGED_AGENT_WORKER_SECRET_STORE_FILE` overrides the worker secret store path written by Themis; by default it targets sibling `../themis-worker-node/infra/local/worker-secrets.json`.
