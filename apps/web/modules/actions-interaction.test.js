...
test("approval action 会在提交后调用 /api/tasks/actions 并清掉等待态", async () => {
  ...
  const turn = {
    ...
    pendingActionError: "旧错误",
    pendingActionSubmitting: true,
  };

  await interaction.submitApproval(turn, "approve");
  assert.equal(calls.length, 1);
  assert.equal(turn.pendingActionError, "");
  assert.equal(turn.pendingActionSubmitting, false);
  assert.equal(turn.pendingAction, null);
  assert.equal(turn.state, "running");
  assert.equal(turn.submittedPendingActionId, "approval-1");
});

test("submitApproval 失败时不会提前清掉 waiting action", async () => {
  const interaction = createActionInteraction({
    submitAction: async () => {
  ...
  await assert.rejects(() => interaction.submitApproval(turn, "approve"), /service unavailable/);
  assert.equal(turn.pendingAction?.actionId, "approval-1");
  assert.equal(turn.state, "waiting");
  assert.equal(turn.pendingActionError, "");
  assert.equal(turn.pendingActionSubmitting, true);
  assert.equal(turn.submittedPendingActionId, null);
});
...
test("user-input action 会在提交后调用 /api/tasks/actions 并带上 inputText", async () => {
  ...
  assert.equal(turn.pendingAction, null);
  assert.equal(turn.state, "running");
  assert.equal(turn.submittedPendingActionId, "input-1");
});
