...
test("initialize 会先修复本地中断 turn，再恢复待同步线程并继续拉历史", async () => {
  ...
  try {
    ...
    assert.equal(dom.composerActionBar.listeners.click.length, 1);
...
test("恢复中的会话 banner 点击 open-thread 会复用 sidebar 切换逻辑", async () => {
  const harness = createActionsHarness({
  ...
  try {
    ...
    await dom.threadRiskBanner.listeners.click[0]({
      target: {
        closest() {
          return {
            dataset: {
              riskBannerAction: "open-thread",
    ...
    await waitFor(() => app.store.state.activeThreadId === restoreThread.id);
    await waitFor(() => calls.sessionSettingsLoad.some((entry) => entry.threadId === restoreThread.id));

    assert.equal(app.store.state.activeThreadId, restoreThread.id);
    assert.ok(calls.sessionSettingsLoad.some((entry) => entry.threadId === restoreThread.id));
  } finally {
    harness.restore();
  }
...
test("thread control join toggle 点击会切换展开态，并在展开时 focus 输入框", () => {
  const harness = createActionsHarness();
  ...
  try {
    ...
    assert.equal(app.runtime.threadControlJoinOpen, false);
    assert.equal(dom.conversationLinkInput.focusCalls, 0);

    dom.threadControlJoinToggle.listeners.click[0]();

    assert.equal(app.runtime.threadControlJoinOpen, true);
    assert.equal(dom.conversationLinkInput.focusCalls, 1);
...
test("activateThread 和 newThreadButton 成功路径都会收起 thread control join panel", async () => {
  ...
  try {
    const { app, dom, localThread, restoreThread, actions } = harness;

    app.runtime.threadControlJoinOpen = true;
    actions.initialize();

    await dom.threadList.listeners.click[0]({
      target: {
        closest() {
          return {
            dataset: {
    ...
    await dom.newThreadButton.listeners.click[0]();

    assert.equal(app.runtime.threadControlJoinOpen, false);
    assert.notEqual(app.store.state.activeThreadId, localThread.id);
  } finally {
    harness.restore();
  }
});
...
test("waiting approval 卡片点击批准会直接提交 decision", async () => {
  ...
  try {
    ...
    await waitFor(() => getLatestTurn(app, restoreThread.id)?.pendingAction?.actionType === "approval");
...
test("waiting action 的 submit 事件会从真实表单解析 thread/turn 并提交 inputText", async () => {
  const harness = createActionsHarness({
  ...
  try {
    ...
    const form = createWaitingActionForm({
      threadId: restoreThread.id,
      turnId: "task-restore",
...
test("initialize 恢复 waiting action 后，经过第二个 action 与再次刷新仍能沿同一条恢复链收口", async () => {
  ...
  try {
    ...
    const firstWaitingActionForm = createWaitingActionForm({
      threadId: restoreThread.id,
      turnId: "task-restore",
      inputText: "第一次恢复回复",
    });
  ...
  try {
    ...
    const secondWaitingActionForm = createWaitingActionForm({
      threadId: restoreThread.id,
      turnId: "task-restore",
      inputText: "第二次恢复回复",
    });
    ...
    await dom.conversation.listeners.submit[0]({
      ...
      target: {
        closest(selector) {
          if (selector === 'form[data-turn-action-kind="waiting"]') {
            return secondWaitingActionForm;
          }
...
function createActionsHarness(options = {}) {
  ...
  if (reusePersistedState) {
    ...
    app.store.state = {
      activeThreadId: restoreScenario === "waiting-approval" ? localThread.id : restoreThread.id,
      threads: [restoreThread, localThread],
    };

    if (restoreScenario === "waiting-approval") {
      restoreThread.historyNeedsRehydrate = true;
    }
          ...
          turns: [
            {
              ...
              events: [
                {
                  eventId: "event-restore-waiting-approval-1",
                  requestId: "req-restore",
                  taskId: "task-restore",
                  type: "task.action_required",
                  status: "waiting",
...
function createDomHarness() {
  return {
    ...
    threadControlJoinToggle: createEventHost("thread-control-join-toggle"),
    ...
    threadRiskBanner: createEventHost("thread-risk-banner"),
  };
}
