...
test("ManagedAgentPlatformGatewayClient 会按 work-items 契约读写平台协作接口", async () => {
      ...
      return jsonResponse({
        ...
        latestCompletion: {
          ...
          structuredOutput: {
            ...
            artifactPaths: ["/srv/project-alpha/report.md"],
            artifactContents: {
              stdout: {
                label: "标准输出",
          ...
          completedAt: "2026-04-13T10:08:00.000Z",
        },
        collaboration: {
          parentWorkItem: null,
  ...
  assert.equal(
    detail?.latestCompletion?.structuredOutput?.deliverable,
    "这里是完整交付正文。",
  );
  assert.equal(
    (detail?.latestCompletion?.structuredOutput?.artifactContents as { stdout?: { content?: string } } | undefined)
      ?.stdout?.content,
    "worker stdout line 1",
  );
...
test("ManagedAgentPlatformGatewayClient 会按 mailbox 与 runs 契约读写平台协作视图", async () => {
  ...
  const fetchImpl: typeof fetch = async (input, init) => {
    ...
    if (url.endsWith("/api/platform/runs/detail")) {
      return jsonResponse({
        ...
        completionResult: {
          summary: "run-alpha 已完成并生成报告。",
          structuredOutput: {
            reportFile: "/srv/project-alpha/report.json",
            artifactContents: {
              stderr: {
                label: "标准错误",
          ...
          completedAt: "2026-04-13T10:25:00.000Z",
        },
      });
    }
  ...
  assert.equal(runDetail?.completionResult?.summary, "run-alpha 已完成并生成报告。");
  assert.equal(runDetail?.completionResult?.detailLevel, "full_execution_snapshot");
  assert.equal(
    runDetail?.completionResult?.structuredOutput?.reportFile,
    "/srv/project-alpha/report.json",
  );
  assert.equal(
    (runDetail?.completionResult?.structuredOutput?.artifactContents as { stderr?: { content?: string } } | undefined)
      ?.stderr?.content,
    "worker stderr warning",
  );
