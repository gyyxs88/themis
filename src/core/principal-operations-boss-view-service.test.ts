...
test("getBossView 会避免对已有风险和节奏的 watch 资产继续提示补风险或节奏", () => {
  const root = mkdtempSync(join(tmpdir(), "themis-operations-boss-view-"));
  const registry = new SqliteCodexSessionRegistry({
    databaseFile: join(root, "infra/local/themis.db"),
  });
  ...
  try {
    ...
    assetsService.createAsset({
      principalId,
      assetId: "asset-betternovels",
      kind: "domain",
      name: "betternovels.com",
      status: "watch",
      now: "2026-04-23T11:00:00.000Z",
    });
    risksService.createRisk({
      principalId,
      riskId: "risk-betternovels-520",
      type: "incident",
      title: "betternovels.com 520 事故已收口",
      severity: "medium",
      status: "resolved",
      relatedAssetIds: ["asset-betternovels"],
      detectedAt: "2026-04-22T10:00:00.000Z",
      now: "2026-04-22T10:00:00.000Z",
    });
    cadencesService.createCadence({
      principalId,
      cadenceId: "cadence-betternovels-monthly",
      title: "betternovels.com 离线资产月度复查",
      frequency: "monthly",
      status: "active",
      nextRunAt: "2026-05-22T10:00:00.000Z",
      relatedAssetIds: ["asset-betternovels"],
      now: "2026-04-23T11:05:00.000Z",
    });
    ...
    const assetFocusItem = bossView.focusItems.find((item) => item.objectId === "asset-betternovels");

    assert.equal(assetFocusItem?.actionLabel, "复查观察结论");
    assert.equal(assetFocusItem?.summary, "该资产处于 watch，已有风险和节奏事实，适合复查观察结论。");
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
