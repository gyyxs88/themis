...
test("ToolTraceTimeline 可以把所有未终态行收口成 interrupted", () => {
  ...
  assert.match(timeline.renderActiveBucket() ?? "", /\n\n2\. 中断 cmd-2/);
});

test("ToolTraceTimeline 会把过长的工具细节截到 50 个字符", () => {
  const timeline = new ToolTraceTimeline({ maxEntries: 10, maxEdits: 12 });

  const rendered = timeline.apply(createInput({
    opId: "op-long-1",
    label: "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789",
    phase: "started",
  })).text;

  assert.match(rendered, /1\. 正在运行 abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWX\.\.\./);
  assert.doesNotMatch(rendered, /YZ0123456789/);
});
