...
test("ManagedAgentCoordinationService 会在命中 projectId 时优先继承项目工作区绑定", () => {
  const { root, registry, managedAgentsService, coordinationService } = createServiceContext();
  ...
  try {
    ...
    const frontend = managedAgentsService.createManagedAgent({
      ...
      departmentRole: "前端",
      mission: "负责官网项目开发。",
      now: "2026-04-13T08:11:00.000Z",
    });
    const projectWorkspace = join(root, "workspace/site-foo");
    ...
    managedAgentsService.upsertProjectWorkspaceBinding({
      ...
      canonicalWorkspacePath: projectWorkspace,
      continuityMode: "sticky",
      now: "2026-04-13T08:12:00.000Z",
    });
    ...
    const dispatched = coordinationService.dispatchWorkItem({
      ...
      goal: "沿用既有项目工作区继续开发。",
      now: "2026-04-13T08:13:00.000Z",
    });

    assert.equal(dispatched.workItem.projectId, "project-site-foo");
    assert.equal(dispatched.workItem.workspacePolicySnapshot?.workspacePath, projectWorkspace);
    assert.equal(dispatched.workItem.workspacePolicySnapshot?.displayName, "项目工作区绑定");
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
