...
test("AppServerTaskRuntime 会在 prompt 里明确注入员工治理工具使用说明", async () => {
  ...
  try {
    ...
    assert.match(state.turns[0]?.prompt ?? "", /manage_themis_secret as Themis' private password book/);
    assert.match(state.turns[0]?.prompt ?? "", /cloudflare-account-id/);
...
test("AppServerTaskRuntime 会把历史 sdk thread 归并成 app-server thread 并直接恢复", async () => {
  const { state, sessionFactory } = createSessionFactory({
    startThreadId: "thread-app-new-engine",
    resumeThreadId: "thread-sdk-old-1",
  });
  const fixture = createRuntimeFixture({ sessionFactory });
  ...
  try {
    ...
    assert.equal(state.started.length, 0);
    assert.equal(state.resumed.length, 1);
    assert.equal(state.resumed[0]?.threadId, "thread-sdk-old-1");
    assert.equal(readSessionPayload(result).threadId, "thread-sdk-old-1");
  } finally {
    fixture.cleanup();
  }
});

test("AppServerTaskRuntime 会把没有 engine 标记的 legacy session 归并成 app-server thread 并恢复", async () => {
  const { state, sessionFactory } = createSessionFactory({
    startThreadId: "thread-app-legacy-new",
    resumeThreadId: "thread-legacy-old-1",
  });
  const fixture = createRuntimeFixture({ sessionFactory });
  ...
  try {
    ...
    assert.equal(state.started.length, 0);
    assert.equal(state.resumed.length, 1);
    assert.equal(state.resumed[0]?.threadId, "thread-legacy-old-1");
    assert.equal(readSessionPayload(result).threadId, "thread-legacy-old-1");
  } finally {
    fixture.cleanup();
  }
});
...
test("AppServerTaskRuntime 会把旧 sdk 会话保留的 thread 直接视为 app-server，并允许后续 review", async () => {
  const { state, sessionFactory } = createSessionFactory({
    startThread: async () => {
  ...
  try {
    ...
    assert.equal(state.started.length, 0);
    assert.equal(state.resumed.length, 1);
    assert.equal(readSessionPayload(result).threadId, "thread-sdk-legacy-fail-1");

    await fixture.runtime.startReview({
      sessionId: "web-session-mixed-fail-1",
      instructions: "should now be treated as app-server",
    });
    assert.deepEqual(state.reviews, [{
      threadId: "thread-sdk-legacy-fail-1",
      instructions: "should now be treated as app-server",
...
test("AppServerTaskRuntime 会把历史 sdk 会话在 startReview 时直接接到现有 thread", async () => {
  const { state, sessionFactory } = createSessionFactory();
  const fixture = createRuntimeFixture({ sessionFactory });
  ...
  try {
    ...
    await fixture.runtime.startReview({
      sessionId: "web-session-sdk-review-1",
      instructions: "should attach to normalized app-server thread",
    });
...
test("AppServerTaskRuntime 会把 MCP 工具审批 reverse request 转成等待中的 action，并在提交后回包", async () => {
  ...
  try {
    ...
    assert.equal(actionRequired.actionType, "approval");
    assert.match(actionRequired.prompt ?? "", /cancel_scheduled_task|MCP 工具调用需要审批|Need approval/);
...
test("AppServerTaskRuntime 会自动批准 Themis managed-agent MCP 工具审批，且不会发 action_required，也不会留下等待审批的 tool trace", async () => {
  ...
  try {
    seedCompletedPrincipalPersona(fixture.runtime, {
      channel: "feishu",
      channelUserId: "feishu-user-managed-tool-approval",
      displayName: "数字员工主管",
    });

    await fixture.runtime.runTask({
      requestId: "req-app-managed-tool-approval-1",
      taskId: "task-app-managed-tool-approval-1",
      sourceChannel: "feishu",
      user: { userId: "feishu-user-managed-tool-approval" },
      goal: "please auto-approve managed-agent MCP tool",
      channelContext: { channelSessionKey: "feishu-session-managed-tool-approval-1" },
...
test("AppServerTaskRuntime 会自动批准 list_scheduled_tasks，且不会发 action_required，也不会留下等待审批的 tool trace", async () => {
  const { state, sessionFactory } = createSessionFactory({
    startTurn: async (sessionState) => {
      sessionState.serverRequestHandler?.({
        id: "server-scheduled-list-tool-approval-1",
        method: "mcpServer/elicitation/request",
        params: {
          serverName: "themis_scheduled_tasks",
          mode: "form",
      ...
      setTimeout(() => {
        sessionState.notificationHandler?.({
          method: "item/completed",
          params: {
            threadId: "thread-app-scheduled-list-tool-approval-1",
            turnId: "turn-app-scheduled-list-tool-approval-1",
            item: {
              type: "mcpToolCall",
              id: "item-scheduled-list-tool-call-1",
              server: "themis_scheduled_tasks",
      ...
      setTimeout(() => {
        scheduleCompletedTurn(sessionState, "turn-app-scheduled-list-tool-approval-1", {
          threadId: "thread-app-scheduled-list-tool-approval-1",
        });
      }, 40);
      return { turnId: "turn-app-scheduled-list-tool-approval-1" };
    },
  });
  const fixture = createRuntimeFixture({
    sessionFactory,
  ...
  try {
    seedCompletedPrincipalPersona(fixture.runtime, {
      channel: "feishu",
      channelUserId: "feishu-user-scheduled-list-tool-approval",
      displayName: "数字员工主管",
    });

    await fixture.runtime.runTask({
      requestId: "req-app-scheduled-list-tool-approval-1",
      taskId: "task-app-scheduled-list-tool-approval-1",
      sourceChannel: "feishu",
      user: { userId: "feishu-user-scheduled-list-tool-approval" },
      goal: "please auto-approve scheduled task listing",
      channelContext: { channelSessionKey: "feishu-session-scheduled-list-tool-approval-1" },
    ...
    assert.equal(sawActionRequired, false);
    assert.deepEqual(toolProgresses, [
      "工具轨迹\n1. 已调用 MCP themis_scheduled_tasks.list_scheduled_tasks",
    ]);
    assert.deepEqual(state.respondedServerRequests, [{
      id: "server-scheduled-list-tool-approval-1",
      result: {
...
test("AppServerTaskRuntime 会把非白名单 MCP elicitation 审批转成等待中的 action，并在提交后回包", async () => {
    ...
    startTurn: async (sessionState) => {
      sessionState.serverRequestHandler?.({
        ...
        params: {
          serverName: "themis_scheduled_tasks",
          mode: "form",
          message: "Allow the themis_scheduled_tasks MCP server to run tool \"cancel_scheduled_task\"?",
          requestedSchema: {
            type: "object",
            properties: {},
          },
          _meta: {
            codex_approval_kind: "mcp_tool_call",
            tool_title: "Cancel Scheduled Task",
          },
        },
      });
      await approvalResolved.promise;
  ...
  try {
    ...
    assert.equal(actionRequired.actionType, "approval");
    assert.match(actionRequired.prompt ?? "", /cancel_scheduled_task|Cancel Scheduled Task|themis_scheduled_tasks/);
