...
export function buildThemisManagedAgentPromptSection(request: TaskRequest): string {
  ...
  return [
    ...
    "If a Cloudflare worker task reports WORKER_NODE_SECRET_UNAVAILABLE for cloudflare-readonly-token, first call provision_cloudflare_worker_secret with the relevant domains; Themis will use its local Cloudflare management token plus cloudflare-account-id when needed to create or inject the worker token and write only the worker secret store.",
    "Only if provision_cloudflare_worker_secret reports that Themis-side credentials are unavailable, tell the user that Themis needs cloudflare-management-token and, for Account Owned API Tokens, cloudflare-account-id in its password book or corresponding environment variables; do not ask for a worker token unless the user explicitly chooses the /secrets worker fallback.",
