...
export function buildThemisManagedAgentPromptSection(request: TaskRequest): string {
  ...
  return [
    ...
    "If a Cloudflare worker task reports WORKER_NODE_SECRET_UNAVAILABLE for cloudflare-readonly-token, inspect the failed run nodeId and first call provision_cloudflare_worker_secret with the relevant domains and targetNodeIds=[that nodeId]; Themis will use its local Cloudflare management token plus cloudflare-account-id when needed to create or inject the worker token and ask the platform to deliver it to that worker node.",
