...
test("waiting action 不是终态，同一任务恢复后还能继续推送进度和下一轮 waiting", async () => {
  const operations: string[] = [];
  let nextMessageId = 1;
  const bridge = createBridge(operations, () => `message-${nextMessageId++}`);

  await bridge.prepareResponseSlot();
  await bridge.deliver(createWaitingActionMessage(
    "req-multi-waiting",
    "第一轮等待：请完成应用配置。\n使用 /reply action-1 <内容>",
    "action-1",
  ));
  await bridge.deliver(createCommentaryProgressMessage(
    "req-multi-waiting",
    "item-after-first-waiting",
    "应用配置已经成功落盘，继续第二步授权。",
  ));
  await bridge.deliver(createWaitingActionMessage(
    "req-multi-waiting",
    "第二轮等待：请完成登录授权。\n使用 /reply action-2 <内容>",
    "action-2",
  ));
  await bridge.deliver(createResultMessage("req-multi-waiting", "授权完成，表格已经建好。"));

  assert.deepEqual(operations, [
    "create:处理中...",
    "update:message-1:第一轮等待：请完成应用配置。\n使用 /reply action-1 <内容>",
    "create:处理中...",
    "update:message-2:应用配置已经成功落盘，继续第二步授权。",
    "create:处理中...",
    "update:message-3:第二轮等待：请完成登录授权。\n使用 /reply action-2 <内容>",
    "create:处理中...",
    "update:message-4:授权完成，表格已经建好。",
  ]);
});
...
function createWaitingActionMessage(requestId: string, text: string, actionId: string) {
  return {
    kind: "event" as const,
    requestId,
    title: "task.action_required",
