...
test("飞书自然语言交付飞书 Open Platform 凭据会保存到固定 secretRef", async () => {
  const harness = createHarness();
  const appId = "cli_a924f3bc42b89bc7";
  const appSecret = "feishu-secret-1234567890abcdef1234567890";

  try {
    await harness.handleRawMessageEvent(createFeishuTextEvent({
      chatId: "chat-1",
      userId: "user-1",
      messageId: "message-feishu-app-id",
      text: `这个是飞书 App ID，给你保存好：${appId}`,
    }));
    await harness.handleRawMessageEvent(createFeishuTextEvent({
      chatId: "chat-1",
      userId: "user-1",
      messageId: "message-feishu-app-secret",
      text: `这个是飞书 App Secret，给你保存好：${appSecret}`,
    }));

    const messages = harness.takeMessages().join("\n");
    assert.match(messages, /secretRef：feishu-app-id/);
    assert.match(messages, /secretRef：feishu-app-secret/);
    assert.doesNotMatch(messages, new RegExp(appId));
    assert.doesNotMatch(messages, new RegExp(appSecret));
    assert.deepEqual(harness.readThemisSecretStore(), {
      "feishu-app-id": appId,
...
test("/ops 会展示实例运维命令", async () => {
  ...
  try {
    ...
    assert.match(message, /\/ops status 查看当前实例状态/);
...
test("/ops status 会展示当前实例状态和最近重启请求", async () => {
  const harness = createHarness({
    updateService: {
      async readOverview() {
        throw new Error("not used");
      ...
      readOpsStatus() {
        return {
          processStartedAt: "2026-04-24T03:50:00.000Z",
          currentCommit: "1234567890abcdef",
          currentCommitSource: "git",
          currentBranch: "main",
          serviceUnit: "themis-prod.service",
          restartPlanMessage: "重启 systemd --user 服务 themis-prod.service。",
          restartPlanErrorMessage: null,
          serviceStatus: {
            serviceUnit: "themis-prod.service",
            loadState: "loaded",
            activeState: "active",
            subState: "running",
            mainPid: 1234,
            execMainStartTimestamp: "Fri 2026-04-24 11:50:00 CST",
            errorMessage: null,
          },
          restartRequest: {
            requestId: "restart-1",
            ...
            status: "confirmed",
            serviceUnit: "themis-prod.service",
            requestedAt: "2026-04-24T03:49:00.000Z",
            requestedCommit: "1234567890abcdef",
            requestedCommitSource: "git",
            requestedProcessStartedAt: "2026-04-24T03:40:00.000Z",
            initiatedBy: null,
            confirmedAt: "2026-04-24T03:50:05.000Z",
            confirmedCommit: "1234567890abcdef",
            confirmedCommitSource: "git",
            confirmedProcessStartedAt: "2026-04-24T03:50:00.000Z",
            confirmedProcessId: 1234,
            failedAt: null,
            errorMessage: null,
          },
        };
  ...
  try {
    await harness.handleCommand("ops", ["status"]);

    const message = harness.takeSingleMessage();
    assert.match(message, /Themis 实例状态：/);
    assert.match(message, /当前提交：1234567（git）/);
    assert.match(message, /服务状态：active \(running\)，LoadState=loaded/);
    assert.match(message, /最近重启请求：已确认/);
    assert.match(message, /请求重启：\/ops restart confirm/);
  } finally {
    harness.cleanup();
  }
});
...
test("/ops restart 在缺少 confirm 时只返回安全提示，不会请求重启", async () => {
  ...
  try {
    ...
    assert.match(message, /查看状态：\/ops status/);
    assert.deepEqual(calls, []);
  } finally {
    harness.cleanup();
  }
...
test("/ops restart confirm 会请求重启当前 Themis 服务", async () => {
  const calls: Array<{ serviceUnitOverride?: string | null; initiatedBy?: unknown }> = [];
  const harness = createHarness({
    updateService: {
      async readOverview() {
        throw new Error("not used");
  ...
  try {
    ...
    assert.deepEqual(calls, [{
      serviceUnitOverride: "themis-prod.service",
      initiatedBy: {
        channel: "feishu",
        channelUserId: "user-1",
        displayName: "user-1",
  ...
  try {
    ...
    const delivered = await harness.notifyManagedAgentScheduledFollowupResolved({
      ...
      latestCompletion: {
        ...
        output: { checkedZone: "novelrift.com", status: "active" },
        touchedFiles: ["reports/cloudflare-readonly.md"],
        detailLevel: "full_execution_snapshot",
        interpretationHint: "当前 completion 已包含交付正文和完整执行快照。",
        structuredOutput: {
          deliverable: "Cloudflare 只读巡检交付：zone novelrift.com 正常，A 记录与台账一致。",
          artifactContents: {
            stdout: {
              content: "zone=novelrift.com status=active",
            },
          },
        },
        completedAt: "2026-04-22T08:15:00.000Z",
    ...
    assert.match(requests[0]?.goal ?? "", /completion detailLevel：full_execution_snapshot/);
    assert.match(requests[0]?.goal ?? "", /completion output：\{\"checkedZone\":\"novelrift.com\",\"status\":\"active\"\}/);
    assert.match(requests[0]?.goal ?? "", /completion touchedFiles：reports\/cloudflare-readonly.md/);
    assert.match(requests[0]?.goal ?? "", /completion deliverable：Cloudflare 只读巡检交付/);
    assert.match(requests[0]?.goal ?? "", /completion artifactContents keys：stdout/);
...
test("tool trace progress 不会被飞书 service 改写成任务状态更新摘要面", async () => {
  const harness = createHarness({
    runtimeEngine: "app-server",
    appServerRuntimeFactory: ({
      runtimeStore,
      ...
      async runTask(request, hooks = {}) {
        taskRuntimeCalls.appServer += 1;
        await hooks.onEvent?.({
          eventId: "event-feishu-tool-trace-1",
          taskId: request.taskId ?? "task-feishu-tool-trace-1",
          requestId: request.requestId,
          type: "task.progress",
          status: "running",
          ...
          payload: {
            traceKind: "tool",
            traceBucketId: "tool-bucket-1",
          },
          timestamp: new Date().toISOString(),
        });
        await hooks.onEvent?.({
          eventId: "event-feishu-tool-trace-2",
          taskId: request.taskId ?? "task-feishu-tool-trace-1",
          requestId: request.requestId,
          type: "task.progress",
          status: "running",
  ...
  try {
    await harness.handleIncomingText("展示工具轨迹");

    const messages = harness.takeMessages();
    assert.ok(messages.some((message) => message.includes("工具轨迹")));
    assert.ok(messages.some((message) => message.includes("已运行 npm run build")));
    assert.doesNotMatch(messages.join("\n"), /任务状态更新/);
    assert.doesNotMatch(messages.join("\n"), /系统继续处理中/);
  } finally {
    harness.cleanup();
...
test("飞书 commentary completed 在最终一致时会保留运行中占位并以已完成收口", async () => {
  const harness = createHarness({
    runtimeEngine: "app-server",
    appServerRuntimeFactory: ({
      runtimeStore,
...
test("飞书公开任务会使用外层进度间超时，不再把总时长 timeoutMs 透传给 runtime", {
  timeout: 400,
}, async () => {
  const harness = createHarness({
    runtimeEngine: "app-server",
    ...
    appServerRuntimeFactory: ({
      ...
      async runTask(request, hooks = {}) {
        taskRuntimeCalls.appServer += 1;
        assert.equal(hooks.timeoutMs, undefined);

        await new Promise((resolve) => setTimeout(resolve, 10));
        await hooks.onEvent?.({
          eventId: "event-feishu-outer-timeout-keepalive-1",
          taskId: request.taskId ?? "task-feishu-outer-timeout-keepalive-1",
          requestId: request.requestId,
          type: "task.progress",
          status: "running",
          message: "第一段进度",
          payload: {
            itemType: "agent_message",
            threadEventType: "item.completed",
            itemId: "item-feishu-outer-timeout-keepalive-1",
          },
          timestamp: new Date().toISOString(),
        });
        ...
        await hooks.onEvent?.({
          eventId: "event-feishu-outer-timeout-keepalive-2",
          taskId: request.taskId ?? "task-feishu-outer-timeout-keepalive-1",
          requestId: request.requestId,
          type: "task.progress",
          status: "running",
  ...
  try {
    await harness.handleIncomingText("请验证飞书公开任务的进度间超时");
    const messages = harness.takeMessages();
    assert.ok(messages.includes("最终完成"));
    assert.doesNotMatch(messages.join("\n"), /任务因超时被取消/);
  } finally {
    harness.cleanup();
  }
...
test("飞书公开任务在持续静默时会由外层进度间超时取消", {
      ...
      async runTask(_request, hooks = {}) {
        ...
        await new Promise<never>((_resolve, reject) => {
          const abort = (): void => {
            ...
            reject(hooks.signal?.reason ?? new Error("aborted"));
          };
  ...
  try {
    await harness.handleIncomingText("请验证飞书公开任务的静默超时");
    const messages = harness.takeMessages();
    assert.match(messages.join("\n"), /任务因超时被取消/);
  } finally {
    harness.cleanup();
...
test("飞书 commentary completed 会立即露出完整正文，并在最终一致时以已完成收口", async () => {
  const harness = createHarness({
    runtimeEngine: "app-server",
    appServerRuntimeFactory: ({
      runtimeStore,
      ...
      async runTask(request, hooks = {}) {
        ...
        const progressText = [
          ...
          "",
          "- 要点一：第一站是翻译站，不是聚合站。",
          "- 要点二：流量入口来自 Novel Updates，而不是泛流量广告。",
          "- 要点三：这条已经足够作为当前轮讨论底稿。",
          "- 要点四：你们当时的策略是围绕入口和转化路径做的选择。",
        ].join("\n");

        await hooks.onEvent?.({
          eventId: "event-feishu-eager-progress-1",
          taskId: request.taskId ?? "task-feishu-eager-progress-1",
          requestId: request.requestId,
          type: "task.progress",
          status: "running",
          ...
          payload: {
            itemType: "agent_message",
            threadEventType: "item.completed",
            itemId: "item-feishu-eager-progress-1",
          },
          timestamp: new Date().toISOString(),
        });
  ...
  try {
    await harness.handleIncomingText("请复现飞书立即露出首条完整正文");

    const messages = harness.takeMessages();
    assert.equal(messages[0], "处理中...");
...
test("飞书多条 commentary completed 会各自落成独立正文气泡，并在最终一致时只收口状态占位", async () => {
  const harness = createHarness({
    runtimeEngine: "app-server",
    appServerRuntimeFactory: ({
      runtimeStore,
      ...
      async runTask(request, hooks = {}) {
        taskRuntimeCalls.appServer += 1;
        const itemId = "item-feishu-streaming-same-item-1";

        await hooks.onEvent?.({
          eventId: "event-feishu-streaming-same-item-1a",
          taskId: request.taskId ?? "task-feishu-streaming-same-item-1",
          requestId: request.requestId,
          type: "task.progress",
          status: "running",
          message: "第一版",
        ...
        await hooks.onEvent?.({
          eventId: "event-feishu-streaming-same-item-1b",
          taskId: request.taskId ?? "task-feishu-streaming-same-item-1",
          requestId: request.requestId,
          type: "task.progress",
          status: "running",
  ...
  try {
    await harness.handleIncomingText("请复现飞书同一条正文的节拍更新");
...
test("飞书 commentary completed 会直接推送完整正文，并在最终一致时只收口为已完成", async () => {
  const harness = createHarness({
    runtimeEngine: "app-server",
    appServerRuntimeFactory: ({
      runtimeStore,
      ...
      async runTask(request, hooks = {}) {
        taskRuntimeCalls.appServer += 1;
        const fullProgress = "第一句已经完整。第二句还在继续展开但是现在还没有写完";

        await hooks.onEvent?.({
          eventId: "event-feishu-soft-boundary-1",
          taskId: request.taskId ?? "task-feishu-soft-boundary-1",
          requestId: request.requestId,
          type: "task.progress",
          status: "running",
          message: fullProgress,
          payload: {
            itemType: "agent_message",
            threadEventType: "item.completed",
            itemId: "item-feishu-soft-boundary-1",
          },
          timestamp: new Date().toISOString(),
        });
  ...
  try {
    await harness.handleIncomingText("请复现飞书按句末截断的 soft flush");

    const messages = harness.takeMessages();
    assert.deepEqual(messages, [
      "处理中...",
      "第一句已经完整。第二句还在继续展开但是现在还没有写完",
      "处理中...",
      "已完成",
    ]);
  } finally {
...
test("飞书 commentary completed 不会再等待 hard flush，而是直接推送完整正文", async () => {
  const harness = createHarness({
    runtimeEngine: "app-server",
    appServerRuntimeFactory: ({
      runtimeStore,
      ...
      async runTask(request, hooks = {}) {
        taskRuntimeCalls.appServer += 1;
        const fullProgress = "这是一段没有句号也没有空行但是仍然需要在四十秒兜底时刷出来让用户知道系统确实还在继续产出";

        await hooks.onEvent?.({
          eventId: "event-feishu-hard-boundary-1",
          taskId: request.taskId ?? "task-feishu-hard-boundary-1",
          requestId: request.requestId,
          type: "task.progress",
          status: "running",
          message: fullProgress,
          payload: {
            itemType: "agent_message",
            threadEventType: "item.completed",
            itemId: "item-feishu-hard-boundary-1",
          },
          timestamp: new Date().toISOString(),
        });
  ...
  try {
    await harness.handleIncomingText("请复现飞书 40 秒 hard flush");

    const messages = harness.takeMessages();
    assert.deepEqual(messages, [
      "处理中...",
      "这是一段没有句号也没有空行但是仍然需要在四十秒兜底时刷出来让用户知道系统确实还在继续产出",
      "处理中...",
      "已完成",
    ]);
  } finally {
...
test("飞书 commentary completed 不再等待结构化补全，短开头也会立刻露出", async () => {
  const harness = createHarness({
    runtimeEngine: "app-server",
    appServerRuntimeFactory: ({
      runtimeStore,
      ...
      async runTask(request, hooks = {}) {
        taskRuntimeCalls.appServer += 1;
        const itemId = "item-feishu-partial-same-item-1";
        ...
        await hooks.onEvent?.({
          eventId: "event-feishu-partial-same-item-1a",
          taskId: request.taskId ?? "task-feishu-partial-same-item-1",
          requestId: request.requestId,
          type: "task.progress",
          status: "running",
          message: shortLead,
        ...
        await hooks.onEvent?.({
          eventId: "event-feishu-partial-same-item-1b",
          taskId: request.taskId ?? "task-feishu-partial-same-item-1",
          requestId: request.requestId,
          type: "task.progress",
          status: "running",
  ...
  try {
    await harness.handleIncomingText("请复现飞书短开头不应过早露出");

    const messages = harness.takeMessages();
    assert.equal(messages[0], "处理中...");
    assert.ok(messages.includes("收到，老板。这一段我先按事实收住，不下判断。\n\n关键"));
    assert.ok(messages.includes([
      "收到，老板。这一段我先按事实收住，不下判断。",
      "",
      "关键节点是：",
...
test("飞书在命中旧附件追溯意图时会把 SQLite 里的历史附件摘要带进请求", async () => {
  const harness = createHarness();

  try {
    const attachmentDirectory = join(
      harness.getWorkingDirectory(),
      "temp",
      "feishu-attachments",
      "session-history-a",
      "message-history-a",
    ...
    const attachmentPath = join(attachmentDirectory, "id_ed25519_admin1");
    writeFileSync(attachmentPath, "fake-private-key", "utf8");

    harness.seedHistoricalFeishuAttachment({
      requestId: "req-history-key-1",
      sessionId: "session-history-a",
      name: "id_ed25519_admin1",
      localPath: attachmentPath,
      sourceMessageId: "message-history-a",
      createdAt: "2026-04-20T12:00:00.000Z",
    });

    await harness.handleIncomingText("之前发给你的 id_ed25519_admin1 私钥还在吗");

    const request = harness.getTaskRequests().at(-1);
    assert.ok(request?.additionalPromptSections?.[0]);
    assert.match(request?.additionalPromptSections?.[0] ?? "", /Recovered prior Feishu attachment facts:/);
    assert.match(request?.additionalPromptSections?.[0] ?? "", /id_ed25519_admin1/);
    assert.match(request?.additionalPromptSections?.[0] ?? "", /exists=yes/);
    assert.match(request?.additionalPromptSections?.[0] ?? "", /sessionId=session-history-a/);
  } finally {
    harness.cleanup();
  }
});

test("飞书普通新任务不会因为提到 key 就注入历史附件摘要", async () => {
  const harness = createHarness();

  try {
    await harness.handleIncomingText("帮我生成一个新的 ssh key");

    const request = harness.getTaskRequests().at(-1);
    assert.equal(request?.additionalPromptSections ?? null, null);
  } finally {
    harness.cleanup();
  }
});
...
test("飞书普通新消息遇到运行中任务时会展示打断/排队/取消选择卡", async () => {
  let releaseFirstTask = (): void => {};
  ...
  const harness = createHarness({
    ...
    appServerRuntimeFactory: ({
      ...
      async runTask(request, hooks = {}) {
        taskRuntimeCalls.appServer += 1;
        await hooks.onEvent?.({
          eventId: `event-active-task-choice-${taskRuntimeCalls.appServer}`,
          taskId: request.taskId ?? `task-active-task-choice-${taskRuntimeCalls.appServer}`,
          requestId: request.requestId,
          type: "task.progress",
          status: "running",
          message: request.goal,
        ...
        const result: TaskResult = {
          taskId: request.taskId ?? `task-active-task-choice-${taskRuntimeCalls.appServer}`,
          requestId: request.requestId,
          status: firstTaskAborted ? "cancelled" : "completed",
          summary: request.goal,
          output: request.goal,
          completedAt: new Date().toISOString(),
        };
  ...
  try {
    const first = harness.handleMessageEventText("第一条长任务");
    await firstTaskStarted;

    await harness.handleMessageEventText("第二条新消息");

    const interactiveDraft = harness.peekRenderedMessages()
      .find((entry) => entry.msgType === "interactive" && entry.content.includes("任务运行中"));
    assert.ok(interactiveDraft);
    const decisions = listInteractiveCardButtons(interactiveDraft.content).map((button) => button.value?.decision);
    assert.deepEqual(decisions, ["interrupt", "queue", "cancel"]);
    assert.equal(harness.getTaskRuntimeCalls().appServer, 1);
    assert.equal(firstTaskAborted, false);

    const cancelButton = listInteractiveCardButtons(interactiveDraft.content)
      .find((button) => button.value?.decision === "cancel");
    assert.ok(cancelButton?.value);
    const callbackCard = await harness.handleCardActionEvent({
      ...
      event: {
        ...
        open_message_id: "om_active_task_choice_cancel_1",
        token: "callback-token-active-choice-cancel-1",
        action: {
          tag: "button",
    ...
    assert.match(JSON.stringify(callbackCard), /已取消新消息/);
    assert.equal(harness.getTaskRuntimeCalls().appServer, 1);
    assert.equal(firstTaskAborted, false);
...
test("飞书运行中选择卡选择打断后会中断当前任务并处理新消息", async () => {
  let firstTaskAborted = false;
  let markFirstTaskStarted = (): void => {};
  const firstTaskStarted = new Promise<void>((resolve) => {
    markFirstTaskStarted = resolve;
  ...
  const harness = createHarness({
    ...
    appServerRuntimeFactory: ({
      ...
      async runTask(request, hooks = {}) {
        ...
        await hooks.onEvent?.({
          eventId: `event-active-task-interrupt-${taskRuntimeCalls.appServer}`,
          taskId: request.taskId ?? `task-active-task-interrupt-${taskRuntimeCalls.appServer}`,
          requestId: request.requestId,
          type: "task.progress",
          status: "running",
          message: request.goal,
        ...
        const result: TaskResult = {
          taskId: request.taskId ?? `task-active-task-interrupt-${taskRuntimeCalls.appServer}`,
          requestId: request.requestId,
          status: firstTaskAborted && /第一条/.test(request.goal) ? "cancelled" : "completed",
          summary: request.goal,
          output: request.goal,
          completedAt: new Date().toISOString(),
        };
  ...
  try {
    ...
    const interactiveDraft = harness.peekRenderedMessages()
      .find((entry) => entry.msgType === "interactive" && entry.content.includes("任务运行中"));
    assert.ok(interactiveDraft);
    const interruptButton = listInteractiveCardButtons(interactiveDraft.content)
      .find((button) => button.value?.decision === "interrupt");
    assert.ok(interruptButton?.value);
    ...
    const callbackCard = await harness.handleCardActionEvent({
      ...
      event: {
        ...
        user_id: "user-1",
        tenant_key: "tenant-card-test",
        open_message_id: "om_active_task_choice_interrupt_1",
        token: "callback-token-active-choice-interrupt-1",
        action: {
          tag: "button",
          value: interruptButton.value,
        },
      },
    });
    assert.match(JSON.stringify(callbackCard), /已选择打断/);

    await waitFor(() => firstTaskAborted);
    await first;
    await waitFor(() => seenGoals.includes("第二条新消息"));
    assert.equal(harness.getTaskRuntimeCalls().appServer, 2);
  } finally {
    harness.cleanup();
  }
});
...
test("飞书后续普通文本不会重复消费同一份附件草稿", async () => {
  ...
  let markSecondTaskFinished = (): void => {};
  const secondTaskFinished = new Promise<void>((resolve) => {
    markSecondTaskFinished = resolve;
  });
  ...
  try {
    ...
    await harness.handleMessageEventText("第二条普通文本");
    const interactiveDraft = harness.peekRenderedMessages()
      .find((entry) => entry.msgType === "interactive" && entry.content.includes("任务运行中"));
    assert.ok(interactiveDraft);
    const queueButton = listInteractiveCardButtons(interactiveDraft.content)
      .find((button) => button.value?.decision === "queue");
    assert.ok(queueButton?.value);
    assert.deepEqual(startedAttachmentCounts, []);
    ...
    const callbackCard = await harness.handleCardActionEvent({
      ...
      event: {
        ...
        user_id: "user-1",
        tenant_key: "tenant-card-test",
        open_message_id: "om_active_task_choice_queue_1",
        token: "callback-token-active-choice-queue-1",
        action: {
          tag: "button",
          value: queueButton.value,
        },
      },
    });
    assert.match(JSON.stringify(callbackCard), /已排队/);

    releaseFirstTask();
    await first;
    await secondTaskFinished;
...
test("飞书完成结果里的显式附件指令会回传 IM 文件附件", async () => {
  const harness = createHarness({
    runtimeEngine: "app-server",
    appServerRuntimeFactory: ({
      taskRuntimeCalls,
  ...
  try {
    ...
    assert.doesNotMatch(messages, /themis-feishu-attachments/);
    assert.match(messages, /交接文档我整理好了/);
...
test("飞书完成结果里的显式图片附件指令会回传 IM 图片附件", async () => {
  const harness = createHarness({
    runtimeEngine: "app-server",
    appServerRuntimeFactory: ({
      taskRuntimeCalls,
...
test("飞书完成结果里的普通本地文档链接不会再自动回传 IM 附件", async () => {
  const harness = createHarness({
    runtimeEngine: "app-server",
    appServerRuntimeFactory: ({
      taskRuntimeCalls,
      ...
      async runTask(request, hooks = {}) {
        ...
        const reportPath = join(harness.getWorkingDirectory(), "docs", "handoff.md");
        const result = {
          taskId: request.taskId ?? "task-feishu-outbound-link-1",
          requestId: request.requestId,
          status: "completed" as const,
          summary: "实现完成",
          output: `文档在这里：[handoff](<${reportPath}:3>)`,
          structuredOutput: {
            session: {
              engine: "app-server" as const,
            },
  ...
  try {
    ...
    await harness.handleIncomingText("如果输出里只是普通本地文档链接，不要自动把文档发回去");
...
test("飞书完成结果里的 touchedFiles 不会再自动回传 IM 附件", async () => {
  const harness = createHarness({
    runtimeEngine: "app-server",
    appServerRuntimeFactory: ({
      taskRuntimeCalls,
      ...
      async runTask(request, hooks = {}) {
        taskRuntimeCalls.appServer += 1;
        const result = {
          taskId: request.taskId ?? "task-feishu-outbound-temp-1",
          requestId: request.requestId,
          status: "completed" as const,
          summary: "图片已生成",
          output: "图片已经生成，但这次我不主动作为附件发送。",
  ...
  try {
    ...
    await harness.handleIncomingText("如果只是 runtime touchedFiles，不要自动把图片发到飞书");
  ...
  return {
    ...
    createTaskPayload(sessionId: string, text: string) {
      return (service as unknown as {
        createTaskPayload(
          incomingContext: typeof context,
          currentSessionId: string,
        ): { options?: Record<string, unknown>; additionalPromptSections?: string[] };
      }).createTaskPayload({ ...context, text }, sessionId);
    },
    seedHistoricalFeishuAttachment(input: {
      requestId: string;
      sessionId: string;
      userId?: string;
      name: string;
      ...
      runtimeStore.saveTurnInput({
        requestId: input.requestId,
        envelope: {
          envelopeId: `env-${input.requestId}`,
          sourceChannel: "feishu",
          parts: [
            {
              partId: `part-${input.requestId}`,
              type: "document",
              role: "user",
              order: 1,
              assetId: `asset-${input.requestId}`,
            },
          ],
          assets: [
            {
