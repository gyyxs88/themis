...
test("飞书首条足够完整的正文会立即露出，并在最终一致时以已完成收口", async () => {
  const harness = createHarness({
    runtimeEngine: "app-server",
    appServerRuntimeFactory: ({
      runtimeStore,
...
test("飞书对同一条正文的持续输出会按节拍更新已显示消息，并保留状态占位直到结束", async () => {
  const previousTimeout = process.env.FEISHU_PROGRESS_FLUSH_TIMEOUT_MS;
  process.env.FEISHU_PROGRESS_FLUSH_TIMEOUT_MS = "20";
...
test("飞书 20 秒 soft flush 会优先截到最近句末，再在最终结果时补齐后文", async () => {
  const previousTimeout = process.env.FEISHU_PROGRESS_FLUSH_TIMEOUT_MS;
  process.env.FEISHU_PROGRESS_FLUSH_TIMEOUT_MS = "20";
...
test("飞书 40 秒 hard flush 会在没有句末或空行时兜底露出正文", async () => {
  const previousTimeout = process.env.FEISHU_PROGRESS_FLUSH_TIMEOUT_MS;
  process.env.FEISHU_PROGRESS_FLUSH_TIMEOUT_MS = "20";
...
test("飞书不会把同一条正文的短开头过早露出，而会等到结构化补全后再显示", async () => {
  const harness = createHarness({
    runtimeEngine: "app-server",
    appServerRuntimeFactory: ({
      runtimeStore,
