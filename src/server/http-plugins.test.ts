...
test("POST /api/plugins/upgrade 会升级 Codex marketplace", async () => {
  await withPluginsServer(async ({ baseUrl, runtime, authHeaders }) => {
    const identity = runtime.getIdentityLinkService().ensureIdentity({
      channel: "web",
      channelUserId: "browser-5",
      displayName: "Themis Web er-5",
    });
    const service = runtime.getPrincipalPluginsService() as ReturnType<AppServerTaskRuntime["getPrincipalPluginsService"]> & {
      upgradePluginMarketplaces?: ReturnType<AppServerTaskRuntime["getPrincipalPluginsService"]>["upgradePluginMarketplaces"];
    };

    service.upgradePluginMarketplaces = async (principalId, input, options) => {
      assert.equal(principalId, identity.principalId);
      assert.equal(input?.marketplaceName, "openai-curated");
      assert.equal(options?.cwd, "/workspace/demo");
      return {
        target: {
          targetKind: "auth-account",
    ...
    const response = await postJson(baseUrl, "/api/plugins/upgrade", {
      channel: "web",
      channelUserId: "browser-5",
      displayName: "Themis Web er-5",
      cwd: "/workspace/demo",
    ...
    assert.deepEqual(payload.result?.selectedMarketplaces, ["openai-curated"]);
    assert.deepEqual(payload.result?.upgradedRoots, ["/tmp/openai-curated"]);
  });
});
